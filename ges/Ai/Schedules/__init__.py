...
class Cond( GEAiCond.Cond ):
    ...
    GES_CAN_NOT_SEEK_ARMOR	= BaseCondition()
    ...
    GES_NO_TARGET			= BaseCondition()
    GES_LOW_AMMO			= BaseCondition()
    GES_AMMO_AVAILABLE		= BaseCondition()
...
class Sched( GEAiSched.Sched ):
    ...
    BOT_COMBAT_FACE = c.BotCombatFace()
    BOT_RANGE_ATTACK1 = c.BotRangeAttack1()
    BOT_PATROL = c.BotPatrol()
    BOT_SEEK_ENEMY = c.BotSeekEnemy()
    BOT_TAKE_COVER_FROM_ENEMY = c.BotTakeCoverFromEnemy()
    BOT_ENGAGE_ENEMY = c.BotEngageEnemy()
    BOT_SEEK_WEAPON = c.BotSeekWeapon()
    BOT_SEEK_WEAPON_DETERMINED = c.BotSeekWeaponDetermined()
    BOT_SEEK_AMMO = c.BotSeekAmmo()
    BOT_SEEK_AMMO_DETERMINED = c.BotSeekAmmoDetermined()
    BOT_SEEK_ARMOR = c.BotSeekArmor()
    BOT_SEEK_ARMOR_DETERMINED = c.BotSeekArmorDetermined()
