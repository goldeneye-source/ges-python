...
class TournamentDM( GEScenario ):
	...
	def OnCVarChanged( self, name, oldvalue, newvalue ):
		if name == "tdm_fraglimit":
			...
			if val != self.FragLimit:
				if val == 0:
					GEUtil.HudMessage( None, "The frag limit was disabled!", -1, -1, GEUtil.Color( 255, 255, 255, 255 ), 4.0 )
				else:
					GEUtil.HudMessage( None, "The frag limit was changed to %i!" % val, -1, -1, GEUtil.Color( 255, 255, 255, 255 ), 4.0 )
				# Finally, set the fraglimit
				self.FragLimit = val
	...
	def OnPlayerSpawn( self, player ):
		if player.IsInitialSpawn():
			if self.FragLimit >= 2:
				GEUtil.PopupMessage( player, "Tournament DM", "The first team to score " + str( self.FragLimit ) + " frags wins." )
			elif self.FragLimit == 1:
				GEUtil.PopupMessage( player, "Tournament DM", "The first team to score a frag wins." )
			else:
	...
	def OnThink( self ):
		...
		if GERules.GetNumActiveTeamPlayers( GEGlobal.TEAM_MI6 ) < 2 or GERules.GetNumActiveTeamPlayers( GEGlobal.TEAM_JANUS ) < 2:
			...
			if not self.warmupTimer.HadWarmup():
				self.warmupTimer.StartWarmup( int( GEUtil.GetCVarValue( "tdm2_warmuptime" ) ), True )
				if self.warmupTimer.inWarmUp:
					GEUtil.EmitGameplayEvent( "tdm2_startwarmup" )
			else:
				GERules.EndRound( False )
...
# /////////////////////////// Utility Functions ///////////////////////////
	...
	def PrintFragMessages( self, killer, teamName, teamScore ):
		if not self.FirstBlood and self.FragLimit > 1:
			GEUtil.ClientPrint( None, GEGlobal.HUD_PRINTTALK, "%s ^1got ^rfirst blood." % teamName )
		...
		if not self.LateAttendance:
			...
			if scoreToWin == 0:
				GEUtil.ClientPrint( None, GEGlobal.HUD_PRINTTALK, "%s ^1has won the match." % teamName )
				self.LateAttendance = True
			elif scoreToWin <= 3:
				count = plural2( scoreToWin, "frag", "frags" )
				GEUtil.ClientPrint( None, GEGlobal.HUD_PRINTTALK, "%s ^1needs ^y%i ^1more %s." % ( teamName, scoreToWin, count ) )
				self.PlayTeamSounds( killer.GetTeamNumber() )
